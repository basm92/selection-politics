...
# Phase 4: per-candidacy electoral environment (district-era race margins/
# runoff structure/RD closeness; PR-era list position and a party-strength
# proxy -- reads only data/panel/panel.duckdb, no scraping; writes
# data/panel/electoral_environment.parquet)
...
|---|---|
| `data/panel/*.parquet` (unified 1848-1937 `candidates_panel` + `persons_post1917`, `elections_post1917`, `gekozen_unmatched`, `mp_anchor`, `mp_anchor_unmatched`, `candidate_roster`, `candidate_person_pairs`, `candidate_person_pairs_summary`, `beroep_hisco_matches`, `dynasty_edges`, `dynasty_candidates`, `candidate_status`, `electoral_environment`) + `data/panel/panel.duckdb` | `data/delpher/` — PDFs (~1.3 GB) + `delpher.duckdb` (OCR/parse tables) |
